...
#define DEVICE_NAME                     'H', 'O', 'G', 'P', 'D', 'E', 'M', 'O'
...
// HID Keyboard scan codes for A, B, C, D, E
#define HID_KEY_A    0x04
#define HID_KEY_B    0x05
#define HID_KEY_C    0x06
#define Left_Shift_modifier 0x02
#define modifier_clear 0x00
...
static void ble_evt_handler(ble_evt_param_t *p_param)
{
    ...
    case BLE_SM_EVT_NUMERIC_COMPARISON:
    {
        ...
        // Auto-confirm pairing without user input
        param.host_id = p_cfm_param->host_id;
        param.same_numeric = 1;  // Always accept (1 = matched)
        
        info_color(LOG_CYAN, "Auto-confirming pairing code: %06d\n", p_cfm_param->comparison_value);
        ble_cmd_numeric_comp_result_set(&param);
#endif
    }
    break;
...
/* Handle key event from GPIO scan task */
void app_key_event_handler(uint8_t key_code)
{
    // Only process if connected
    if (ble_app_link_info[APP_HID_P_HOST_ID].state == STATE_CONNECTED)
    {
        g_current_key_code = key_code;
        
        // Send key event to application queue
        if (app_request_set(APP_HID_P_HOST_ID, APP_REQUEST_KEY_PRESS, true) == false)
        {
...
void passkey_set(uint8_t *p_data, uint8_t length)
{
    ...
    if (ble_numeric_comp_state == 1)
    {
        ...
        printf("Numeric Comparison result: %d\n", same_numeric);
        // send HIDS passkey
