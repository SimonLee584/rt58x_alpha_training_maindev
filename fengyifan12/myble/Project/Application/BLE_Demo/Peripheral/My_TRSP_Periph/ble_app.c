...
#define DEVICE_NAME                     'T', 'R', 'S', 'P', '_', 'F'
...
static const ble_gap_addr_t  DEVICE_ADDR = {.addr_type = RANDOM_STATIC_ADDR,
                                            .addr = {0x01, 0x02, 0x03, 0x24, 0x25, 0xC6 }
                                           };
...
static void ble_svcs_trsps_evt_handler(ble_evt_att_param_t *p_param)
{
    if (p_param->gatt_role == BLE_GATT_ROLE_SERVER)
    {
        ...
        case BLESERVICE_TRSPS_UDATRW01_WRITE_WITHOUT_RSP_EVENT:
        {
            ...
            uint16_t cccd = p_profile_info->svcs_info_trsps.server_info.data.udatni01_cccd;
            ...
            if ((cccd & BLEGATT_CCCD_NOTIFICATION) != 0)
            {
                ble_err_t status = ble_svcs_trsps_server_send(
                    p_param->host_id,
                ...
                if (status == BLE_ERR_OK)
                    info_color(LOG_GREEN, "Echo sent via NOTIFICATION\n");
                else
                    info_color(LOG_RED, "Echo send failed (err=%d)\n", status);
            }
            else if ((cccd & BLEGATT_CCCD_INDICATION) != 0)
            {
                ble_err_t status = ble_svcs_trsps_server_send(
                    p_param->host_id,
                ...
                if (status == BLE_ERR_OK)
                    info_color(LOG_GREEN, "Echo sent via INDICATION\n");
                else
                    info_color(LOG_RED, "Echo send failed (err=%d)\n", status);
            }
            else
            {
                info_color(LOG_YELLOW, "CCCD not enabled, cannot echo\n");
            }
        }
        break;
        ...
        break;
        case BLESERVICE_CUSTOM_CUSTDATA01_READ_EVENT:
        {
            ble_err_t status;
            const uint8_t report_data[] = "CustomData01_Value";
            ble_gatt_data_param_t gatt_data_param;
            
            gatt_data_param.host_id = p_param->host_id;
            gatt_data_param.handle_num = p_param->handle_num;
            gatt_data_param.length = sizeof(report_data) - 1;
            gatt_data_param.p_data = (uint8_t *)report_data;
        ...
        case BLESERVICE_CUSTOM_CUSTCMD01_WRITE_EVENT:
        case BLESERVICE_CUSTOM_CUSTCMD01_WRITE_WITHOUT_RSP_EVENT:  // CUSTCMD01 Write without response
        {
            uint8_t *p_data;
            p_data = pvPortMalloc(p_param->length + 1);
            if (p_data != NULL)
            ...
            uint16_t cccd = 0;
            uint16_t handle_num = 0;
            
            // 获取Custom服务CCCD和句柄
            if (ble_svcs_custom_info_get(p_param->host_id, &cccd, &handle_num) == BLE_ERR_OK)
            {
                
                // 检查CCCD，如果启用了通知或指示，则回显数据
                if ((cccd & BLEGATT_CCCD_NOTIFICATION) != 0)
                {
                    ble_err_t status = ble_svcs_custom_server_send(
                        p_param->host_id,
                        BLEGATT_CCCD_NOTIFICATION,
                        handle_num,
                        p_param->data,
...
static void app_peripheral_handler(app_req_param_t *p_param)
{
    ...
    case APP_REQUEST_ADV_START:
        do
        {
            ...
            // Initialize Custom service with callback handler
            ble_svcs_custom_init(host_id, ble_svcs_trsps_evt_handler);
...
void app_init(void)
{
    ...
    info_color(LOG_DEFAULT, "  BLE TRSP (P) demo: start...\n");
    info_color(LOG_DEFAULT, "------------------------------------------\n");
