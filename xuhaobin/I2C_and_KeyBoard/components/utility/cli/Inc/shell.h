...
#define sh_cmd_add(cmd_name, desc_, cmd_func, priority)                        \
    static void __attribute__((constructor(priority))) SHELL_CMD_UID(          \
        cmd_reg_helper_)() {                                                   \
 ...
 *
 */
#define SHELL_CALC_HISTORY_BUFFER(line_size, cmd_deep)                         \
    (((line_size) + 4) * (cmd_deep))
...
/**
 * @brief process the shell command string.
 *
 * @param input Pointer of the input string.
 * @return int 0 will be returned if the deinitialate was successfully
 */
