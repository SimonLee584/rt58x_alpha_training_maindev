...
#define err(str, ...)                                                          \
    do {                                                                       \
        utility_printf("[%s:%u] " str, __func__, __LINE__, ##__VA_ARGS__);     \
        while (1)                                                              \
            ;                                                                  \
    } while (0)
#else
...
//=============================================================================
static sh_dev_t g_sh_dev = {
    .pIO = 0,
};
static sh_cmd_t* g_pCmd_head = 0;

static sh_esc_item_t g_esc_code_table[] = {
    {.esc_code = SH_ESC_CODE_HOME,
     .pEsc_ascii = ESC_CODE_HOME,
     .esc_code_len = 4},
    {.esc_code = SH_ESC_CODE_INSERT,
     .pEsc_ascii = ESC_CODE_INSERT,
     .esc_code_len = 4},
    {.esc_code = SH_ESC_CODE_DELETE,
     .pEsc_ascii = ESC_CODE_DEL,
     .esc_code_len = 4},
    {.esc_code = SH_ESC_CODE_DELETE,
     .pEsc_ascii = ESC_CODE_VT100_DELETE,
     .esc_code_len = 3},
    {.esc_code = SH_ESC_CODE_END,
     .pEsc_ascii = ESC_CODE_END,
     .esc_code_len = 4},
    {.esc_code = SH_ESC_CODE_PAGE_UP,
     .pEsc_ascii = ESC_CODE_PAGE_UP,
     .esc_code_len = 4},
    {.esc_code = SH_ESC_CODE_PAGE_DOWN,
     .pEsc_ascii = ESC_CODE_PAGE_DOWN,
     .esc_code_len = 4},
    {.esc_code = SH_ESC_CODE_UP, .pEsc_ascii = ESC_CODE_UP, .esc_code_len = 3},
    {.esc_code = SH_ESC_CODE_DOWN,
     .pEsc_ascii = ESC_CODE_DOWN,
     .esc_code_len = 3},
    {.esc_code = SH_ESC_CODE_LEFT,
     .pEsc_ascii = ESC_CODE_LEFT,
     .esc_code_len = 3},
    {.esc_code = SH_ESC_CODE_RIGHT,
     .pEsc_ascii = ESC_CODE_RIGHT,
     .esc_code_len = 3},
    {.esc_code = SH_ESC_CODE_BACKSPACE,
     .pEsc_ascii = ESC_CODE_BACKSPACE,
     .esc_code_len = 1},
    {.esc_code = SH_ESC_CODE_TAB,
     .pEsc_ascii = ESC_CODE_TAB,
     .esc_code_len = 1},
    {.esc_code = SH_ESC_CODE_CTRL_C,
     .pEsc_ascii = ESC_CODE_CTRL_C,
     .esc_code_len = 1},
    {
...
//=============================================================================
static int _shell_log(const char* fmt, ...) {
    ...
    // va_end(args);
    if (pIO && pIO->cb_write)
        rval = pIO->cb_write((uint8_t*)log_buf, strlen(log_buf),
                             g_sh_dev.pUser_data);

    return rval;
}

static int _shell_push_to_history(char* pLine) {
...
static char* _shell_pop_history(int* pRow_index) {
    char* pLine = 0;
...
static int _shell_erase_line(uint32_t* pWr_pos, uint32_t* pCursor_pos,
    ...
    // delete characters
    while (wr_pos--) {
        cb_write((uint8_t*)ESC_CODE_LEFT ESC_CODE_VT100_DELETE, 6,
                 g_sh_dev.pUser_data);
    }
...
static int _shell_get_esc_code(char character, sh_esc_code_t* pEsc_code) {
    sh_esc_item_t* pEsc_item_cur = g_esc_code_table;

    if ((uint8_t)character == ESC_START_CODE
        || g_sh_dev.esc_input_cumulation
               >= sizeof(g_sh_dev.esc_buf.esc_op_buf)) {
    ...
    while (pEsc_item_cur->esc_code != SH_ESC_CODE_TOTAL) {
        ...
        if (!strncmp(pEsc_item_cur->pEsc_ascii, g_sh_dev.esc_buf.esc_op_buf,
...
__unused static int
_shell_esc_code_proc(sh_esc_code_t esc_code, uint32_t* pWr_pos,
    ...
    switch (esc_code) {
        ...
        case SH_ESC_CODE_HOME: {
            if (*pCursor_pos > 0) {
                ...
                cb_write((uint8_t*)str_buf, strlen(str_buf),
                         g_sh_dev.pUser_data);
                *pCursor_pos = 0;
            }
            return 0;
        ...
        case SH_ESC_CODE_END: {
            if (*pCursor_pos < *pWr_pos) {
                char str_buf[12] = {0};
                snprintf(str_buf, 12, "\x1b[%dC",
                         (int)((*pWr_pos) - (*pCursor_pos)));
                cb_write((uint8_t*)str_buf, strlen(str_buf),
                         g_sh_dev.pUser_data);
                *pCursor_pos = *pWr_pos;
            }
        ...
        case SH_ESC_CODE_DOWN: {
            ...
            g_sh_dev.row_index = (esc_code == SH_ESC_CODE_UP)
                                     ? g_sh_dev.row_index - 1
                                     : g_sh_dev.row_index + 1;
            return 0;
        }
        ...
        case SH_ESC_CODE_DELETE: {
            if (*pCursor_pos < *pWr_pos) {
                ...
                cb_write((uint8_t*)ESC_CODE_VT100_DELETE, 3,
                         g_sh_dev.pUser_data);
...
static sh_state_t _shell_read_line(sh_io_desc_t* pIO) {
    ...
    do {
...

        {
            ...
            if (esc_code == SH_ESC_CODE_TOTAL) {
                ...
                if (g_sh_dev.cursor_pos < wr_pos) {
                    char str_buf[12] = {0};
                    snprintf(str_buf, 12, "\x1b[%dD",
                             (int)(g_sh_dev.wr_pos - g_sh_dev.cursor_pos - 1));
                    cb_write((uint8_t*)str_buf, strlen(str_buf),
                             g_sh_dev.pUser_data);
                }
                ...
                _shell_esc_code_proc(esc_code, &g_sh_dev.wr_pos,
                                     &g_sh_dev.cursor_pos, cb_write);
            }
        }
...
int shell_exec_string(const char* input) {
    if (!input || !*input || !g_sh_dev.pLine_buf || g_sh_dev.line_buf_len == 0)
        return -1;

    char* line_buf = g_sh_dev.pLine_buf;
    memset(line_buf, 0, g_sh_dev.line_buf_len);
    strncpy(line_buf, input, g_sh_dev.line_buf_len - 1);

    int arg_cnt = 0;
    char* pCmd_args[CONFIG_SH_CMD_ARG_MAX_NUM] = {0};
    char* pCur = line_buf;
    char* pEnd = line_buf + strlen(line_buf);
    uint32_t is_arg_head = 1;
    ...
    // execute command
    if (arg_cnt) {
        sh_cmd_t* pCmd_cur = NULL;
        sh_cmd_t* pCmd_act = NULL;
        ...
        if (!pCmd_act) {
            ...
            while (pCmd_cur) {
                if (!strncmp(pCmd_cur->pCmd_name, pCmd_args[0], 3)) {
                    pCmd_act = pCmd_cur;
                    break;
                }
        ...
        if (pCmd_act)
            return pCmd_act->cmd_exec(arg_cnt, pCmd_args, _shell_log,
                                      pCmd_act->pExtra);
    }
...
static int _sh_cmd_help(int argc, char** argv, cb_shell_out_t log_out,
                        void* pExtra) {
    uint32_t cli_cmd_pool_start = (uint32_t)SHELL_CLI_CMD_POOL_START;
    uint32_t cli_cmd_pool_end = (uint32_t)SHELL_CLI_CMD_POOL_END;
...
//=============================================================================
int shell_init(sh_io_desc_t* pDesc, sh_set_t* pSet_info) {
    int rval = 0;
    if (!pDesc) {
        rval = -1;
        err("input NULL data\r\n");
    ...
    if (pSet_info->history_buf_size < SHELL_CALC_HISTORY_BUFFER(
            pSet_info->line_size, pSet_info->cmd_deep)) {
        rval = -3;
...
int shell_deinit(void* pInfo) { return 0; }

int shell_register_cmd(sh_cmd_t* pCmd) {
    if (g_pCmd_head) {
        sh_cmd_t* pCur = g_pCmd_head;
        while (pCur->next)
            pCur = pCur->next;
...
void* shell_proc(sh_args_t* pArg) {
    sh_io_desc_t* pIO = g_sh_dev.pIO;
    ...
    do {
        ...
        if (pIO) {
            if (is_line_head) {
                if (pIO->cb_write)
                    pIO->cb_write((uint8_t*)SHELL_PROMPT, sizeof(SHELL_PROMPT),
                                  g_sh_dev.pUser_data);
                is_line_head = 0;
            }
            ...
            if (rval == SH_STATE_GET_LINE) {
                ...
                if (arg_cnt) {
                    sh_cmd_t* pCmd_cur = 0;
                    sh_cmd_t* pCmd_act = 0;
                    uint32_t cli_cmd_pool_start =
                        (uint32_t)SHELL_CLI_CMD_POOL_START;
                    uint32_t cli_cmd_pool_end =
                        (uint32_t)SHELL_CLI_CMD_POOL_END;
                    ...
                    if (pCmd_act)
                        pArg->exec_result = pCmd_act->cmd_exec(
                            arg_cnt, pCmd_args, _shell_log, pCmd_act->pExtra);
                }
