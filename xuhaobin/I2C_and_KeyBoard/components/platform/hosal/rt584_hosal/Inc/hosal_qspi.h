...
/**
 * \defgroup        HOSAL_QSPI Hosal qspi
 * \ingroup         RT584_HOSAL
 * \brief           Define Hosal qspi definitions, structures, and functions
 * @{
 */
...
typedef enum {
    ...
    HOSAL_QSPI_MODE_SET,
    HOSAL_QSPI_MODE_GET,
    HOSAL_QSPI_PRE_DLY_SET,
    HOSAL_QSPI_PRE_DLY_GET,
    HOSAL_QSPI_INTER_DLY_SET,
    HOSAL_QSPI_INTER_DLY_GET,
    HOSAL_QSPI_POST_DLY_SET,
    HOSAL_QSPI_POST_DLY_GET,
...
typedef enum {
    HOSAL_QSPI_PRE_DEALY_DISABLE = 0,
	HOSAL_QSPI_PRE_DEALY_ENABLE = 1,
} hosal_qspi_pre_delay_t;

typedef enum {
    HOSAL_QSPI_INTER_DEALY_DISABLE = 0,
	HOSAL_QSPI_INTER_DEALY_ENABLE = 1,
} hosal_qspi_inter_delay_t;

typedef enum {
    HOSAL_QSPI_POST_DEALY_DISABLE = 0,
	HOSAL_QSPI_POST_DEALY_ENABLE = 1,
} hosal_qspi_post_delay_t;
...
typedef struct {
    ...
    uint8_t data2;
    uint8_t data3;
    uint8_t pre_dly_count;
    uint8_t inter_dly_count;
    uint8_t post_dly_count;
    uint8_t mcs_en;
	...
	hosal_qspi_pre_delay_t  pre_dly;
	hosal_qspi_inter_delay_t inter_dly;
	hosal_qspi_post_delay_t post_dly;
} hosal_qspi_config_t;
...
/**
 * \brief          qspi manual control function.
 * \param[in]      qspi_dev             qspi struct identifier
 * \param[in]      qspi_mcs             manual chip select control
 * \return         hosal_qspi_status_t status
 *
 */   
hosal_qspi_status_t hosal_qspi_mcs_ctrl(hosal_qspi_dev_t* qspi_dev,uint8_t qspi_mcs);
/**
...
/*@}*/ /* end of RT584_HOSAL HOSAL_QSPI */

#ifdef __cplusplus
}
#endif

#endif /* End of HOSAL_QSPI_H */
