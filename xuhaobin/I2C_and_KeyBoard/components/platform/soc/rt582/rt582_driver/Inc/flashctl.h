...
/**
 * \file            flashctl.h
 * \brief           flash contrl header file
 */
...
/**
 * \defgroup FLASHCTL Flash Control
 * \ingroup RT58X_DRIVER
 * \brief  Define Flash Control definitions, structures, and functions
 * @{
 */
...
#define FLASH_STATUS_RW1_2 (FLASH_STATUS_RW1                                    \
                          | FLASH_STATUS_RW2)   /*!< Status Register Byte 1 and Byte 2,
                                                 this define for writing status Bytes2  */
...
#define FLASH_IDLE 0                            /*!< flash idle const define*/
#define FLASH_BUSY 1                            /*!< flash busy const define*/

#define FLASH_SUSPEND_STATUS   0x80             /*!<  flash suspend status const define*/
#define FLASH_BUSY_CHECK_COUNT 10000            /*!<  flash busy check const define*/

#define FLASH_TYPE_ID_MAKS 0xFFFF               /*!<  flash id mask const define*/

#define FLASH_SIZE_ID_SHIFT 16                  /*!<  flash size id const define*/

//limit customer to use
#define BOOT_LOADER_END_PROTECT_ADDR 0x7000     /*!<  Bootloader protect address const define*/

#define FLASH_SIZE_SECTOR_4KB 0x0C              /*!<  flash sector size 4kb const define*/
...
#define LENGTH_4KB  (4 * 1024)                  /*!<  length 4kb const define */
#define LENGTH_32KB (32 * 1024)                 /*!<  length 32kb const define */
#define LENGTH_64KB (64 * 1024)                 /*!<  length 64kb const define */

#define FLASH_END_PROTECT_SIZE 0x2000           /*!<  flash end protect size const define*/

#define GDWQ_ID 0x65C8                          /*!<  GDWQ flash id const define*/
#define GDLQ_ID 0x60C8                          /*!<  GDLQ flash id const define*/
#define PUYA_ID 0x6085                          /*!<  PUYA flash id const define*/

#define GD_MANU_ID   0xC8                       /*!<  GD flash id const define*/
#define PUYA_MANU_ID 0x85                       /*!<  PUYA flash id const define*/

#define RT581_FLASH_TYPE     0x1460C8           /*!<  RT581 1m flash type const define*/
#define RT582_FLASH_TYPE     0x1465C8           /*!<  RT582 1m flash type const define*/
#define RT582P512_FLASH_TYPE 0x136085           /*!<  RT582 512k flash type const define*/
#define RT582_2M_FLASH_TYPE  0x1565C8           /*!<  RT582 1m flash type const define*/

#define FLASH_END_ADDR(x)                                                      \
    (x == FLASH_SIZE_ID_512KB                                                  \
         ? ((1 << FLASH_SIZE_ID_512KB) - FLASH_END_PROTECT_SIZE)               \
         : (x == FLASH_SIZE_ID_1MB                                             \
                ? ((1 << FLASH_SIZE_ID_1MB) - FLASH_END_PROTECT_SIZE)          \
 ...
 */
#define GDLQ_FLASH_TDP 3                        /*!< LQ type flash deep power down (CS# High To
                                                 Deep Power-Down Mode) and the unit is us */
#define GDLQ_FLASH_TRES1 20                     /*!< LQ type flash release deep power down (CS#
                                                 High To Standby Mode Without Electronic Signature)
                                                 and the unit is us */
#define GDLQ_FLASH_TSUS 20                      /*!< LQ type flash suspend time (CS#
                                                 High To Next Command After Suspend)
                                                  and the unit is us */
#define GDLQ_FLASH_TRS 100                      /*!< LQ type flash resume time (Latency
                                                 Between Resume And Next Suspend)
                                                 and the unit is us */

#define GDWQ_FLASH_TDP 3                        /*!< WQ type flash deep power down
                                                 (CS# High To Deep Power-Down Mode)
                                                 and the unit is us */
#define GDWQ_FLASH_TRES1 30                     /*!< WQ type flash release deep power down
                                                 (CS# High To Standby Mode Without 
                                                 Electronic Signature) and the unit is us */
#define GDWQ_FLASH_TSUS 40                      /*!< WQ type flash suspend time 
                                                 (CS# High To Next Command After Suspend) 
                                                 and the unit is us */
#define GDWQ_FLASH_TRS 100                      /*!< WQ type flash resume time
                                                 (Latency Between Resume And Next Suspend)
                                                 and the unit is us */

#define PUYA_FLASH_TDP 3                        /*!< PUYA type flash deep power down 
                                                 (CS# High To Deep Power-Down Mode) and the unit is us */
#define PUYA_FLASH_TRES1 30                     /*!< PUYA type flash release
                                                 deep power down (CS# High To
                                                 Standby Mode Without Electronic Signature) 
                                                 and the unit is us */
#define PUYA_FLASH_TSUS 40                      /*!< PUYA type flash suspend time
                                                 (CS# High To Next Command After Suspend)
                                                 and the unit is us */
#define PUYA_FLASH_TRS 100                      /*!< PUYA type flash resume time
                                                 (Latency Between Resume And Next Suspend) 
                                                 and the unit is us */

/**
* \brief            flash size enum definitions.
*/
typedef enum {
...
/**
 * \brief           flash control erase function
 ...
 */
__STATIC_INLINE uint32_t flash_get_crc() { return (FLASH->crc); }
...
/**
 * \brief           Enable flash contrl suspend
 * \param           NONE
 * \retval          NONE
 */
...
/**
 * \brief           Disable flash contrl suspend
 * \param           NONE
 * \retval          NONE
 */
 ...
 * \param           NONE
 * \retval          Return control register value
 */
uint32_t flash_get_control_reg(void);

/*@}*/ /* end of RT58X_DRIVER FLASHCTL */
